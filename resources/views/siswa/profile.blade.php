    ...
    <div class="height-100">
        ...
        <div class="card-body p-3">
            ...
            <div class="card-text content mt-3 mb-4">
                <form method="POST"
                action="{{ url('siswa/profile', Auth::guard('siswa')->user()->id_siswa) }}">
                @csrf
                <div class="form-group col-md-6 m-2">
                    <label for="courseName">Nama</label>
                    <input name="name" type="text" class="form-control @error('name') is-invalid @enderror"
                        id="courseName" value="{{ Auth::guard('siswa')->user()->name }}"  autofocus>
                    @error('name')
                        {{ $message }}
                    @enderror
                </div>
                <div class="form-group col-md-6 m-2">
                    ...
                    <input name="email" type="text" class="form-control @error('email') is-invalid @enderror"
                        id="email" value="{{ Auth::guard('siswa')->user()->email }}"  >
                    @error('email')
                        {{ $message }}
                    @enderror
                </div>
                <div class="form-group col-md-6 m-2">
                    ...
                    <input name="phoneNumber" type="text" class="form-control @error('phoneNumber') is-invalid @enderror"
                        id="phoneNumber" value="{{ Auth::guard('siswa')->user()->phone_number }}">
                    @error('phoneNumber')
                        {{ $message }}
                    @enderror
                </div>
